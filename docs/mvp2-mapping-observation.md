...
仅证明 Mapping 被保存和读取不够。回归测试使用业务名称与代码名称完全不同的两套同源数据库：控制库不写入 `QUERY_REVIEW` Mapping，实验库确认该 Mapping。相同后续问题必须表现为控制库证据不足、实验库证据充分，并且实验库调查轮次更少。测试不比较执行时间，避免机器负载造成抖动。
