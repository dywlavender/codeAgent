  ...
  return (
    <div className="experiment-setup">
      ...
      <Card className="panel-card launch-card">
        ...
        <div className="launch-mode-row"><span className="field-title">实验模式</span>{ARMS.map((arm) => <div className="mode-pill fixed" key={arm}><Check size={14} weight="bold" /> {ARM_LABEL[arm]}<small>{arm === "code_only" ? "源码、README、需求" : arm === "optional" ? "＋项目总览与业务主干" : "＋结构总览与 AST 索引"}</small></div>)}</div>
