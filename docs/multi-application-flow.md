...
5. Claude Code 会读取工作区中的业务基线、入口提示和当前源码，再在回答中给出业务流程、技术调用链、代码入口和文件位置。

Entry Anchor 只是人工维护的导航提示，不是硬边界，也不是代码事实。入口不存在或同名时，Claude Code 应明确说明并根据当前工作区源码继续搜索；最终结论仍必须以实际文件内容为准。

测试 fixture 位于 `tests/fixtures/multi_application_flow`，与用户示例分离。它不在问题或业务基线中提供类名，测试仍要求检索链找到 H5、渠道服务、贷款中台、HTTP、Feign 和最终处理方法。
