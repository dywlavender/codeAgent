...
如果入口不存在或同名，运行轨迹会记录 `NOT_FOUND` / `MULTIPLE`，仍有入口成功时继续沿成功入口调查；入口全部失效或尚未维护时最多进行一次当前索引搜索，不会把旧入口当成事实。

测试 fixture 位于 `tests/fixtures/multi_application_flow`，与用户示例分离。它不在问题或业务基线中提供类名，测试仍要求 Agent 找到 H5、渠道服务、贷款中台、HTTP、Feign 和最终处理方法。
