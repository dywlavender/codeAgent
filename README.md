...
后续知识主干、词云（知识图谱）及问答调查的整改，以 [知识主干与知识图谱：项目整改指导方向](docs/knowledge-backbone-direction.md) 为依据：帮助模型建立项目整体认识、理解业务全流程及术语关系、掌握调查入口，减少无关探索和关键流程遗漏。下文描述当前实现，不代表该目标已经完成或收益已经验证。

当前内容核查与有／无主干的真实模型对照见 [2026-09-05 验证报告](docs/knowledge-backbone-validation-20260905.md)。本轮尚未证明现有主干能稳定减少调查成本。

后续已完成 [项目总览＋按需流程的两层改写与复测](docs/knowledge-backbone-rewrite-20260905.md)，有局部改善，仍未证明稳定的整体效率优势。
...
查询只启用 `Read`、`Glob`、`Grep`，并禁止 `Edit`、`Write`、`Bash`、`Task` 等修改型工具。运行时通过 `--add-dir` 授权生成的工作区以及其业务基线、需求和仓库链接对应的真实目录；新会话和续聊均应用这些目录。生成的 `CLAUDE.md` 列出绝对搜索路径，避免依赖搜索工具跟随链接。缺失的来源标为不可用，不创建失效链接。
...
每轮问答（含续聊）通过 `--append-system-prompt` 显式传入当前真实资料目录及检索规则，与 `--add-dir` 使用同一份来源清单。跨仓库分别搜索，不自动授权共同上级目录；错误路径可改查已授权目录，正确路径仍被拒绝时报告该来源的访问问题。该约束属于 Agent 提示规则，不是工具调用的强制路径重写。

若业务基线目录存在 `project-overview.md`，每轮同时提供其当前内容，帮助 Agent 了解项目范围、系统职责和业务目录。具体流程文档继续按需读取；总览提供调查地图，当前实现仍需源码确认。编写方式见 [业务基线使用指南](docs/business-baseline-guide.md)。

用户侧“项目资料”展示 Agent 的真实资料目录、目录可读/空/缺失状态及目录授权配置，支持分类筛选、路径复制和刷新。刷新不会拉取代码或导入知识；目录授权配置不等于模型访问实测通过。结构化业务知识在管理员“业务知识维护”中查看，代码索引接口仍保留，但不再作为用户侧一级浏览入口。
...
知识主干的持续验证见[自动对照验证指南](docs/knowledge-backbone-automation.md)：可配置题库，一条命令完成真实模型对照、源码评分初评和报告生成。
